...
public class UserServiceImpl implements UserService {
  ...
  public UserResponse signUp(SignUpRequest request) {
    log.info("회원가입 요청 시작 - username: {}", request.getUsername());

    if (userRepository.existsByUsername(request.getUsername())) {
      log.info("회원가입 실패 - 이미 존재하는 username: {}", request.getUsername());
  ...
  public UserResponse getUserDetail() {
    log.info("사용자 상세 조회 요청 시작");

    User user = getCurrentUser();
    UserCharacter character = userCharacterRepository.findByUserId(user.getId()).orElse(null);

    log.info("사용자 상세 조회 - userId: {}, username: {}", user.getId(), user.getUsername());
    return userMapper.toUserResponse(user, character);
  }
  ...
  public void addPoints(Long userId, AddPointsRequest request) {
    ...
    notificationServiceClient.sendDirect(
        ...
        String.format(
            "+%d포인트 적립! (잔여: %d포인트) - %s",
            request.getPoints(), user.getPoints(), request.getReason()));
  }
  ...
  private User getCurrentUser() {
    ...
    if (principal instanceof String str) {
      ...
      log.info("인증 실패 - Principal 타입 알 수 없음: {}", principal.getClass());
      throw new CustomException(UserErrorCode.UNAUTHORIZED);
    }

    if (username.isBlank()) {
      log.info("인증 실패 - 추출된 username이 빈 문자열");
      throw new CustomException(UserErrorCode.UNAUTHORIZED);
    }
    ...
    return userRepository
        ...
        .orElseThrow(
            () -> {
              log.info("사용자 찾기 실패 - username: {}", username);
