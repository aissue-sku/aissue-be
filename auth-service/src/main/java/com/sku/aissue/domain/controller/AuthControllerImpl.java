  ...
  public ResponseEntity<BaseResponse<String>> testLogin(HttpServletResponse response) {
    UsernamePasswordAuthenticationToken auth =
        new UsernamePasswordAuthenticationToken(
            "aissue", null, List.of(new SimpleGrantedAuthority("ROLE_USER")));

    TokenResponse tokenResponse = jwtProvider.createTokens(auth);
    ...
    return ResponseEntity.status(HttpStatus.OK)
        .body(BaseResponse.success(200, "테스트 로그인이 완료되었습니다.", tokenResponse.getUsername()));
  }
