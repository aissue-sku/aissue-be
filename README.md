...
이미지는 OpenAI `gpt-image-1`로 생성되어 ai-service의 로컬 디스크(`/app/static/issue-cards`, Docker 볼륨)에 저장되고, Gateway의 `/static/issue-cards/**` 경로로 서빙됩니다. **이미지 생성은 비용·시간이 크기 때문에 수동 트리거에서만 실행**합니다.
...
| 로컬 디스크 (Docker 볼륨) | 이슈 카드 이미지 저장 — `/static/issue-cards/**` 로 서빙 |
