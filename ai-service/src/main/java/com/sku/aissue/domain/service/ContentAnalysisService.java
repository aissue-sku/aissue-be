...
public class ContentAnalysisService {
  ...
  private static final String SYSTEM_PROMPT =
      ...
      IMPORTANT: Every "label" and "score" field inside "items" must be filled — never leave them empty or zero unless truly warranted.

      Response format (fill ALL fields exactly as shown):
      {
        "verdict": "신뢰할 수 있음 or 주의 필요 or 신뢰하기 어려움",
        "summary": "2-3 sentences overall summary in Korean",
        "credibility": {
          "score": <sum of items, 0-25>,
          "reason": "...",
          "items": [
        ...
        "factChecks": [{ "claim": "...", "status": "사실 or 불확실 or 거짓", "evidence": "..." }]
      }
      """;
  ...
  private List<RelatedArticle> findRelatedArticles(String title, String currentUrl) {
    ...
    return candidates.stream()
        .filter(c -> c.url() == null || !c.url().equals(currentUrl))
        .limit(3)
        .map(
            c -> {
              Integer trustScore =
                  c.url() != null
                      ? contentAnalysisRepository
                          .findTopByUrlOrderByAnalyzedAtDesc(c.url())
                          .map(ContentAnalysis::getTotalScore)
                          .orElse(estimateSourceScore(c.source()))
                      : estimateSourceScore(c.source());
              return RelatedArticle.builder()
                  .title(c.title())
                  .url(c.url())
                  .publisher(c.source())
                  .timeAgo(toTimeAgo(c.publishedAt()))
